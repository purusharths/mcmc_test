"""
run as:
mpirun -n 5 python3 mcmc_oed.py
"""
...
def unnormalized_log_posterior(Z,w,V,f,obs,i,ssq=1e-4):
    p = -Z.dot(Z)/2
...
if __name__=="__main__":
    ...
    for i in r:
        # log posterior
        log_post = lambda X: unnormalized_log_posterior(X,w,V,f,obs,i,ssq=ssq)

        # vanilla mcmc
        samples,u = mcmc(log_post,d,m,s=0.18)
        data = {"samples":samples,"KL_eigenvalues":w,"KL_eigenmodes":V,"u_values":u}
        pickle.dump(data, open("mcmc_"+str(i)+".p","wb"))
